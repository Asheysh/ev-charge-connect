...
export function WelcomeDashboard() {
  ...
  const queuedSlots = stationQueue.filter((q) => q.status !== "completed" && q.status !== "skipped");
  const yourSlot = stationQueue.find((q) => q.user_id === user.id)?.position ?? queuedSlots.length + 1;
  const currentSlot = stationQueue.find((q) => q.status === "charging")?.position ?? queuedSlots[0]?.position ?? 0;
  ...
  const alerts = useMemo(() => {
    ...
    return [
      ...
      { id: "a2", title: "Queue update", body: `${queuedSlots.length} drivers in queue · avg wait ${waitMin} min.`, ago: "8m ago" },
    ];
  }, [station, queuedSlots.length, waitMin, yourSlot]);
  ...
  const tiles: Array<{ label: string; icon: React.ComponentType<{ className?: string }>; tone: string; onClick?: () => void; to?: string }> = [
    ...
    { label: "Help Support", icon: HelpCircle, tone: "text-primary", onClick: () => setHelpOpen(true) },
    {
      ...
      tone: "text-primary",
      onClick: () => {
        const target = document.getElementById("station-map");
        if (target) {
          target.scrollIntoView({ behavior: "smooth", block: "start" });
          window.history.replaceState({}, "", "#station-map");
        } else {
  ...
  return (
    <section className="grid gap-4">
      ...
      {/* Live status hero */}
      <div className="rounded-[2rem] bg-gradient-to-br from-emerald-600 via-emerald-500 to-sky-500 p-6 text-white shadow-[0_30px_80px_rgba(14,116,144,0.2)]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-emerald-100/90">Live Station Status</p>
            <h2 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">{station?.name ?? "Station Overview"}</h2>
            <p className="mt-1 text-sm text-emerald-100/80">Real-time queue, wait estimate, and slot activity.</p>
          </div>
          <span className="inline-flex items-center rounded-full bg-white/20 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.24em] text-white shadow-sm shadow-white/10">
            Active now
          </span>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <HeroStat label="Your slot" value={yourSlot > 0 ? `#${yourSlot.toString().padStart(2, "0")}` : "—"} icon="#" />
          <HeroStat label="Current" value={currentSlot > 0 ? `#${currentSlot.toString().padStart(2, "0")}` : "—"} icon="⚡" />
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {tiles.map((t) => {
          const Icon = t.icon;
          const inner = (
            <div className="flex min-h-[170px] flex-col items-center justify-center gap-3 rounded-[1.75rem] border border-slate-200/80 bg-white/95 p-5 text-center shadow-[0_18px_50px_rgba(15,23,42,0.08)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_22px_70px_rgba(15,23,42,0.12)]">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-[1.6rem] shadow-sm">
                <Icon className={`${t.tone} size-6`} />
              </div>
...
function HeroStat({ label, value, icon }: { label: string; value: string; icon: string }) {
  return (
    <div className="rounded-[1.5rem] border border-white/15 bg-white/10 p-4 shadow-[0_16px_42px_rgba(255,255,255,0.08)] backdrop-blur-xl">
      <p className="text-[11px] font-semibold uppercase tracking-[0.25em] text-white/80">{label}</p>
      <p className="mt-3 flex items-center gap-3 text-3xl font-black text-white sm:text-4xl">
        <span className="inline-flex h-12 w-12 items-center justify-center rounded-3xl bg-white/15 text-lg text-white shadow-inner">{icon}</span>
        <span>{value}</span>
      </p>
    </div>
  );
